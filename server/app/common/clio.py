...
class Clio():
    ...
    def __init__(self):
        # Read host/port from env (set by Docker Compose) or fall back to Flask config
        try:
            from flask import current_app
            host = current_app.config.get('MONGODB_HOST', 'localhost')
            port = int(current_app.config.get('MONGODB_PORT', 27017))
        except RuntimeError:
    ...
    def _clean_options(cls, opts):
        ...
        if order_by:
            ...
            if order_by not in cls.expected_filters:
                # Cleaned field is not valid.
                order_by = None
            else:
                # Returns the arguments needed by sort() call.
...
class Session(ResourceMixin):
    ...
    def _clean_query(cls, dirty):
        ...
        def clean_integer(field_name, query):
            ...
            # If it's not a proper integer it will be removed
            # from the query.
            try:
    ...
    def attacker_stats(self, ip, hours_ago=None):
        ...
        res = list(self.collection.aggregate(query))
        if len(res) > 0:
            r = res[0]
            del r['_id']
            if r.get('first_seen'):
...
class HpFeed(ResourceMixin):
    ...
    def count_passwords(self, payloads):
        ...
        for creds in payloads:
            if creds.get('credentials') is not None:
    ...
    def count_combos(self, payloads):
        ...
        for combos in payloads:
            if combos.get('credentials') is not None:
