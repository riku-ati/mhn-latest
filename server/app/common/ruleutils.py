...
def _parse_plain(r):
    ...
    option_patterns = (
        ...
        re.compile(r"(sid)\s*:\s*(\d+);"),
        re.compile(r"(rev)\s*:\s*(\d+);"),
        re.compile(r"(reference)\s*:\s*(.*?);"),
        re.compile(r"(classtype)\s*:\s*(.*?);"),
    )
    ...
    for p in option_patterns:
        ...
        for opt, val in p.findall(options):
            ...
            rplctoken = '{' + opt + '};'
            if opt != 'rev':
                # Rev is the last option and doesn't have a trailing space.
                rplctext += ' '
                rplctoken += ' '
...
def from_file(rfile):
    """
    Takes in an opened file object containing rules in plain text and
    creates a list of parsed rules in dict objects.
    """
    rules = []
