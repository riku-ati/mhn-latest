...
from app.ui.utils import get_flag_ip, get_sensor_name
from app.api.models import (
        Sensor, Rule, DeployScript as Script,
        RuleSource)
from app.auth import login_required, current_user
from app.auth.models import User, PasswdReset, ApiKey
from app import db, mhn
...
def get_feeds():
    ...
    channel_list = list(clio.hpfeed.channel_map.keys())
...
def get_credentials_payloads(clio):
    ...
    credentials_payloads += list(clio.hpfeed.get_payloads({'limit': 10000}, {"channel": "kippo.sessions"})[2])
    credentials_payloads += list(clio.hpfeed.get_payloads({'limit': 10000}, {"channel": "cowrie.sessions"})[2])
