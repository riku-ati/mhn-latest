...
def get_pub_ip():
    with urlopen('http://icanhazip.com/') as sock:
        ip = sock.read().decode('utf-8').strip()
    return ip
...
def generate_config():
    ...
    if getattr(args, 'which', 'generate') == 'unattended':
        is_unattended = True
    ...
    localconfig['MAIL_USE_TLS'] = 'y' == mail_tls if isinstance(mail_tls, str) else mail_tls
    localconfig['MAIL_USE_SSL'] = 'y' == mail_ssl if isinstance(mail_ssl, str) else mail_ssl
